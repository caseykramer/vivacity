...
hasConflicts v1 v2 = (Map.size v1 > Map.size v2) || 
    case (v1 \\ v2) of 
...
{- 
    ...
    or a version that is < a node on v1, or is missing one or more
    nodes from v1.  One exception would be a version of 1 on a new 
    node, which means the first version of the object from that node.
-}
